...
# 登录
class loginWithout(View):
    def post(self, request: HttpRequest):
        response_json = create_return_json()
        if (get_json := json.loads(request.body)) is not None:
            unit_id = get_json.get('unit_id', None)
            unit_name = get_json.get('unit_name', None)
            report_date = get_json.get('date')
            response_json['data'] = {'unit_id':unit_id,'unit_name':unit_name}
        else:
            response_json['code'], response_json['msg'] = return_msg.S100, return_msg.params_error
        return JsonResponse(response_json)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.method == 'POST':
            ...
            if file:
                ...
                response_json['data'] = {'id': id, 'name': filename}
                return JsonResponse(response_json)
            else:
    ...
    def post(self, request: HttpRequest, *args, **kwargs):
        ...
        try:
            ...
            with connection.cursor() as cur:
                ...
                rows = rows_as_dict(cur)
                count = rows[0]['count']
                sql = 'select distinct d.id,d.name,d.data as data_info,d.unit_id as unit_id,' \
        ...
        try:
            ...
            with connection.cursor() as cur:
                if 'unit_id' in j:
                    unit_id = j.get('unit_id')
                    params = [unit_id]
                    sql = 'select distinct t.id as id,t.name,t.is_file ' \
